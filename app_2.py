...
class VoiceDetector:
    def __init__(self, chunk=2048, format=pyaudio.paInt16, channels=2, rate=44100, py=pyaudio.PyAudio()):
        ...
        self.btn_select = tk.Button(self.header, text = "Select File", width = 12, command=self.open_file)
        self.btn_select.grid(row=1, column=0, padx = 5, pady = 10)
        self.file_name = tk.Label(self.header, text="No file selected")
        self.file_name.grid(row=2,column=0, padx = 5, pady = 10)
        ...
        # Action bar
        # Record button
        self.action_btn = tk.Button(self.footer, width=10, padx=10, pady=5, text='Start Record', command=lambda: self.record())
        ...
        # Predict button
        self.predict_btn = tk.Button(self.footer, width=5, padx=10, pady=5, text='Predict', command=lambda: self.predict())
    ...
    def predict_function(self):
        ...
        if not self.file_path:
            print("NO file")
            return
        O = self.get_mfcc_predict(self.file_path)
    ...
    def predict(self):
        O = self.get_mfcc(self.file_path)
    ...
    def play_audio(self):
        ...
        self.state_lbl.config(text='Playing '+ str(file_name))
    ...
    def record(self):
        if self.recording == True:
            self.action_btn.config(text="Start Record")
            self.state_lbl.config(text="Waiting for action ...")
            self.recording = False
            self.stop_record()
            self.file_path = "record.wav"
        elif self.recording == False:
            self.action_btn.config(text="Stop Record")
            self.state_lbl.config(text="Recording ...")
            self.recording = True
            self.start_record()
